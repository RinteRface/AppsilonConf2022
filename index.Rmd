...
---
class: header_background
# Why and when do we need validation?

<br/>

.center[
<img src="assets/images/svg/404.svg" width="20%" style="display: block; margin: auto;" class="svg-img"/>
]

--

.small[
In Pharma, we have a mission to improve and extend people's lives. We are responsible for our products' __safety__ and __efficacy__:

- __GxP__ usage (Good clinical practices).
- __DMC__ submission : data monitoring committee with external members.
- ...
]

--

How do we transition from a __prototype__ to a __production-ready__ tool❓

---
class: inverse center middle title-slide-without-bg title-slide
# 2. Production Ready Shiny Apps
...
- __Automated__.
- More __reproducible__.
- __Time saver__.
- Less duplication.

---
class: header_background
# How to activate CI/CD features?
...
- Write __CI/CD instructions__ in yaml file (better support for GitHub).
- Enjoy ...
...
- __Eases__ Shiny Apps validation by __providing__ plug and play CI/CD pipelines:
...
.panel[.panel-name[Flexible]

.center[
```{r shinyvalidator-overview, echo=FALSE, out.width='45%'}
...
- Must be run within a package, especially with `{golem}`.
- Control R package dependencies with `{renv}` (recommended).
- Install it:
.small[
```{r, eval=FALSE}
...
- Run `use_validator()`. Set up all required elements.
